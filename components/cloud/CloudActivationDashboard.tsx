...
export function CloudActivationDashboard() {
  ...
  async function check() {
    setError("");
    try { setHealth(await getCloudHealth()); } catch (e) { setError(e instanceof Error ? e.message : "Cloud health check failed."); }
  }
  ...
  return <main className="min-h-screen bg-[#f5f7fb] px-4 py-6 text-slate-950 sm:px-8">
    <div className="mx-auto max-w-5xl">
      ...
      <section className="mt-5 grid gap-3 sm:grid-cols-3">
        ...
        <Status title="Pilot schema" ready={Boolean(health?.schemaReady)} detail="pilot_students readable" />
      </section>
      <section className="mt-5 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <Link href="/teacher-cloud" className="rounded-2xl bg-amber-500 p-5 font-black text-slate-950">👨‍🏫 Teacher Cloud Setup →</Link>
          <Link href="/cloud-sync" className="rounded-2xl bg-cyan-700 p-5 font-black text-white">☁️ Student Cloud Sync →</Link>
        </div>
      </section>
    </div>
  </main>;
