  ...
  async function uploadAll() {
    ...
    try {
      for (const student of workspace.students) {
        ...
        if (!brain) continue;
        const accessCode = codes[student.profile.id] || makeAccessCode();
        setCodes((current) => ({ ...current, [student.profile.id]: accessCode }));
        await teacherUpsertCloudStudent({ classCode: classCode.trim().toUpperCase(), accessCode, teacherKey, brain });
      }
      setMessage(`✓ Đã kích hoạt ${workspace.students.length} hồ sơ Pilot trên Cloud.`);
      await refreshCloud();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Không thể upload toàn bộ."); }
    ...
    <main className="min-h-screen bg-[#f5f7fb] px-4 py-6 text-slate-950 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <header className="rounded-[2rem] bg-slate-950 p-6 text-white sm:p-8">
          <p className="text-xs font-black uppercase tracking-[0.12em] text-amber-200">Beta 2.6.2 · Cloud Activation</p>
        ...
        <section className="mt-5 grid gap-3 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:grid-cols-2">
          ...
          <div className="sm:col-span-2 flex flex-wrap gap-2">
            <button disabled={busy || uploadingAll || !teacherKey || !classCode} onClick={refreshCloud} className="rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black text-white disabled:opacity-40">↻ Tải danh sách Cloud</button>
            <button disabled={busy || uploadingAll || !teacherKey || !classCode} onClick={uploadAll} className="rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-black text-white disabled:opacity-40">☁ Kích hoạt toàn bộ Pilot</button>
