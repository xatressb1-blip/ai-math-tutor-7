      ...
      <div className="mx-auto max-w-6xl">
        <header className="rounded-[2rem] bg-slate-950 p-6 text-white sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div><p className="text-xs font-black uppercase tracking-[0.12em] text-amber-200">Góc nhìn giáo viên</p>
              <h1 className="mt-3 text-4xl font-black">Tín hiệu trải nghiệm học sinh</h1>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300">Xem dữ liệu học tập được lưu trên chính trình duyệt này.</p>
            </div>
            <Link href="/teacher" className="rounded-2xl bg-white px-4 py-2.5 text-sm font-black text-slate-950">Bảng điều khiển giáo viên →</Link>
          </div>
        </header>
        <section className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Metric label="Phản hồi học tập" value={feedback.length} />
        </section>
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
            ...
            <div className="mt-3 space-y-2">
              {weakest.map((skill) => (
                <div key={skill.id} className="rounded-2xl bg-rose-50 p-4">
                  ...
                  <div className="mt-1 text-xs font-bold text-rose-700">Thành thạo {skill.masteryScore}/100 · Tự tin {skill.confidence}/100</div>
                </div>
              ))}
            </div>
          </section>
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6">
            ...
            <div className="mt-4 space-y-3">
              ...
              {feedback.length === 0 && <p className="text-sm text-slate-500">Chưa có phản hồi học tập.</p>}
            </div>
          </section>
        </div>
      </div>
